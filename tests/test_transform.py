...
class TestSummarize:
    ...
    def test_group_sum(self, sales_df: pd.DataFrame) -> None:
        result = Transform.summarize(
            sales_df, group_by="Region", aggregations={"Revenue": "sum"}
        )
...
class TestCrossTab:
    ...
    def test_long_to_wide(self, sales_df: pd.DataFrame) -> None:
        result = Transform.cross_tab(
            sales_df, group_by="Region", pivot_col="Quarter", value_col="Revenue"
        )
...
class TestCustomAggregations:
    ...
    def test_count_null_and_blank(self) -> None:
        ...
        result = Transform.summarize(
            df, 
            aggregations={
                "Val": ["count null", "count blank", "count non blank"]
            }
        )
    ...
    def test_concatenate(self) -> None:
        ...
        result = Transform.summarize(
            df, aggregations={"Val": ["concatenate", "concatenate distinct"]}
        )
