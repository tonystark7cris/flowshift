...
Flowshift implements all core Flowshift tools grouped into logical palettes:
...
In Flowshift, a tool like `Filter` has a 'True' and 'False' output anchor. In Flowshift, these return as a tuple:
```python
...
- **`Preparation.sample(df, n: int, method: str)`**: Extracts first N, last N, random N, or percent.
  *Usage*: `df = Preparation.sample(df, 100, method="random")`
...
- **`Preparation.generate_rows(df, condition: str, expression: str)`**: Generates sequential rows.
  *Usage*: `df = Preparation.generate_rows(df, "Row < 10", "Row + 1")`
- **`Preparation.auto_field(df)`**: Optimizes data types to save memory footprint.
  *Usage*: `df = Preparation.auto_field(df)`
- **`Preparation.multi_field_formula(df, columns: list, expression: str)`**: Applies one formula across many columns.
  *Usage*: `df = Preparation.multi_field_formula(df, ["Q1", "Q2"], "_CurrentField_ * 1.1")`
- **`Preparation.multi_row_formula(df, column: str, expression: str, group_by: list)`**: Formulas referencing prior/next rows.
  *Usage*: `df = Preparation.multi_row_formula(df, "Running", "[Row-1:Running] + [Sales]", ["Region"])`
...
- **`Preparation.create_samples(df, estimation: float, validation: float, holdout: float)`**: Splits for ML. Returns `(est_df, val_df, hold_df)`.
...
- **`Preparation.oversample_field(df, column: str, value: Any, target_pct: float)`**: Balances target classes via stratified oversampling.
...
- **`Join.join(left, right, on: str)`**: Standard join. Returns `(Left_Unjoined, Joined, Right_Unjoined)`.
...
- **`Join.fuzzy_match(left, right, left_on: str, right_on: str, threshold: float)`**: Approximate string matching.
  *Usage*: `df = Join.fuzzy_match(df1, df2, "CompanyName", "Name", threshold=0.85)`
...
- **Databases**: Flowshift's `InOut` utilizes standard SQLAlchemy and Spark JDBC beneath the hood.
...
### Example 3: Comprehensive End-to-End Enterprise Pipeline
This example demonstrates a complex workflow: extracting raw data, parsing strings with Regex, performing multiple joins, reshaping data (pivoting), and validating the output before saving.
...
def run_enterprise_pipeline():
    # Set backend to spark for big-data processing
    flowshift.set_backend("spark")
    
    # 1. Load Data
    transactions = InOut.input_data("s3://data/raw_transactions.parquet")
    stores = InOut.input_data("s3://data/dim_stores.csv")
    products = InOut.input_data("s3://data/dim_products.json")
    
    # 2. Parse & Cleanse
    # Extract User ID and Domain from Email column using Regex
    transactions = Parse.regex_parse(
        transactions, 
        column="CustomerEmail", 
        pattern=r"(?P<UserID>[^@]+)@(?P<Domain>.+)"
    )
    
    # Standardize string formatting and fill nulls
    transactions = Preparation.data_cleansing(
        transactions, 
        replace_nulls_with="", 
    ...
    # Filter returns a tuple of (True, False). We keep the True records.
    valid_tx, invalid_tx = Preparation.filter(transactions, "Amount > 0 AND Status == 'Complete'")
    
    # 4. Join Multiple Datasets
    # Join transactions with store data
    _, tx_with_stores, _ = Join.join(valid_tx, stores, on="StoreID")
    
    # Fuzzy match product descriptions to normalize product names
    products_matched = Join.fuzzy_match(tx_with_stores, products, "ProductDesc", "ProductName", threshold=0.8)
    
    # Join with product data
    _, enriched_data, _ = Join.join(tx_with_stores, products, on="ProductID")
    
    # 5. Transform & Reshape
    # Calculate weighted average discount per store
    weighted_avg = Transform.weighted_average(
        enriched_data, 
        value_col="Discount", 
        weight_col="Amount", 
        group_by=["Region"]
    ...
    # Pivot the data: Rows=Region, Columns=ProductCategory, Values=Sum(Amount)
    sales_matrix = Transform.cross_tab(
        enriched_data, 
        group_by=["Region"], 
        header_column="ProductCategory", 
        value_column="Amount", 
        aggregation="sum"
    )
    
    # Join weighted average back to the pivoted matrix
    _, final_report, _ = Join.join(sales_matrix, weighted_avg, on="Region", how="inner")
    
    # 6. Validate & Output
    # Ensure no Region has negative sales in the matrix
    Developer.test(final_report, lambda df: df["Sum_Amount"].min() >= 0, "Critical: Negative sales detected after pivot!")
    
    # Output to multiple formats
    InOut.output_data(final_report, "s3://data/reporting/sales_matrix.parquet")
    InOut.output_data(invalid_tx, "s3://data/audit/invalid_transactions.csv")
...
**Current Roadmap (H2):**
- Support for Polars as a third blazing-fast backend engine.
- Out-of-the-box integration with dbt models.
